...
// Siri shortcuts for this intent live in `InventAppShortcuts`
// (MenuAgentIntent.swift) — only one `AppShortcutsProvider` conformance is
// allowed per app, and its builder can't compose from a separate list, so
// all shortcuts (stock and menu) are inlined there directly.
